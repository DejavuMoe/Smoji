...
try {
  ...
  await writeFile(join(live, 'assets/ancient-hash.js'), 'obsolete inherited chunk')
  const previousManifestPath = join(live, '.vite/manifest.json')
  const previousManifest = JSON.parse(await readFile(previousManifestPath, 'utf8'))
  previousManifest.previous = { file: 'assets/previous-hash.js' }
  await writeFile(previousManifestPath, JSON.stringify(previousManifest))
  // Unknown directories and symlinks must never be treated as old releases.
  await mkdir(join(site, 'releases/notes'))
  await symlink(parent, join(site, `releases/${'b'.repeat(40)}-0-0`))
  run(`${sha}-2-0`)
  ...
  await assert.rejects(readFile(join(live, 'assets/ancient-hash.js')), { code: 'ENOENT' })
  ...
  assert((await readFile(join(site, first, 'index.html'))).length > 0, 'Stale publication must not prune history')
  run(`${sha}-2-1`)
  const rerun = await readlink(live)
  assert.equal(rerun, `releases/${sha}-2-1`, 'A newer rerun can activate')
  await assert.rejects(readFile(join(site, first, 'index.html')), { code: 'ENOENT' })
  assert((await readFile(join(site, second, 'index.html'))).length > 0)
  await assert.rejects(readFile(join(live, 'assets/previous-hash.js')), { code: 'ENOENT' })
  assert.deepEqual((await readdir(join(site, 'releases'))).filter(name => name.startsWith(sha)).sort(), [`${sha}-2-0`, `${sha}-2-1`])
  assert.equal(await readlink(join(site, `releases/${'b'.repeat(40)}-0-0`)), parent)
  assert.deepEqual(await readdir(join(site, 'releases/notes')), [])
  ...
  assert((await readFile(join(site, second, 'index.html'))).length > 0, 'Failed publication must keep rollback release')
  ...
  console.log('Publish checks passed: two-release retention, bounded old assets, isolated layout, reruns, stale pipelines and invalid output')
} finally {
