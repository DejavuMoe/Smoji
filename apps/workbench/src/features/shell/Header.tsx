...
export function Header() {
  ...
  return (
    <header className="top sticky top-0 z-30 flex h-14 w-full items-center justify-between border-b border-border bg-surface/95 px-4 backdrop-blur-sm sm:px-6">
      <div className="top__left flex items-center gap-3">
        <a className="brand flex items-center gap-2.5 text-foreground transition-opacity hover:opacity-90" href="/" aria-label="Smoji 首页">
          <img className="brand__mark h-7 w-7" src="/favicon.svg?v=2" width="28" height="28" alt="" />
          <div className="brand__title-group flex items-baseline gap-2">
            <span className="brand__text font-mono text-base font-semibold tracking-tight text-foreground">Smoji</span>
            <span className="brand__description hidden text-xs text-muted-foreground lg:inline">表情工作台</span>
          </div>
        </a>
      </div>

      <div className="top__actions flex items-center gap-1 sm:gap-2">
        {/* Mobile menu toggle */}
        <button
          id="menu-toggle"
          type="button"
        ...
        <button
          id="btn-open-code"
          type="button"
          className="btn-ghost inline-flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-xs font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
